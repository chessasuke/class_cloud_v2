import 'package:class_cloud/common/widgets/cairdio_search_bar.dart';
import 'package:class_cloud/common/widgets/custom_app_bar.dart';
import 'package:class_cloud/core/design_system/text_style.dart';
import 'package:class_cloud/features/coach/screens/coaches_screen/cubit/coach_list_cubit.dart';
import 'package:class_cloud/features/coach/screens/coaches_screen/cubit/coach_list_state.dart';
import 'package:class_cloud/features/coach/screens/coaches_screen/widgets/coach_tile.dart';
import 'package:flutter/material.dart';
import 'package:flutter_bloc/flutter_bloc.dart';

class CoachesScreen extends StatelessWidget {
  const CoachesScreen({super.key});

  @override
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: const CustomAppBar(
        title: 'Coaches',
      ),
      backgroundColor: Colors.white,
      body: RefreshIndicator(
        onRefresh: () => context.read<CoachListCubit>().fetchAllCoaches(),
        child: const _Content(),
      ),
    );
  }
}

class _Content extends StatelessWidget {
  const _Content();

  @override
  Widget build(BuildContext context) {
    return BlocBuilder<CoachListCubit, CoachListState>(
      builder: (context, state) {
        if (state is CoachListLoading) {
          return const Center(child: CircularProgressIndicator());
        } else if (state is CoachListError) {
          return Center(
            child: Text(state.message, style: TextStyles.body01),
          );
        } else if (state is CoachListLoaded) {
          if (state.coaches.isEmpty) {
            return Center(
              child: Text(
                'No Coaches Found',
                style: TextStyles.body01,
              ),
            );
          }
          return Padding(
            padding: const EdgeInsets.all(8.0),
            child: Column(
              children: [
                AppSearchBar(
                  labelText: 'Search Email or Name',
                  onChanged: (value) =>
                      context.read<CoachListCubit>().filterCoaches(value),
                ),
                Expanded(
                  child: ListView.builder(
                    itemCount: state.coaches.length,
                    itemBuilder: (context, index) {
                      return Padding(
                        padding: const EdgeInsets.symmetric(vertical: 16),
                        child: CoachTile(coach: state.coaches[index]),
                      );
                    },
                  ),
                ),
              ],
            ),
          );
        }
        return const SizedBox.shrink();
      },
    );
  }
}
